...
public class PluginChannelTracker {
	...
	private final SetMultimap<String, String> name2channels = MultimapBuilder.hashKeys().hashSetValues().build();

	// A multimap from channel names to the names of players registered to that channel
	private final SetMultimap<String, String> channel2names = MultimapBuilder.hashKeys().hashSetValues().build();
