...
public class ShowBoundingBoxes {
	...
	private static final ArrayList<StructureBox>[] group = new ArrayList[10];
	...
	private static boolean insideRenderDistance(StructureBox box, LocalClientPlayerEntity player) {
		...
		return box.maxZ >= minZ && box.minZ <= maxZ;
	}
	...
	private static boolean isSlimeChunk(int x, int z, long seed) {
		randy.setSeed(seed + (long) ((long) x * x * 4987142) + (long) (x * 5947611L) + (long) ((long) z * z) * 4392871L + (long) (z * 389711L) ^ 987234911L);
